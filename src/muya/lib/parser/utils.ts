...
export interface ParsedSrcAndTitle {
  src: string
  title: string
}

// selected from https://developer.mozilla.org/en-US/docs/Web/HTML/Attributes
export const WHITELIST_ATTRIBUTES: ReadonlyArray<string> = Object.freeze([
...
const validWidthAndHeight = (value: string | null): number | '' => {
  if (value === null || !/^\d{1,}$/.test(value)) return ''
  const parsedValue = parseInt(value, 10)
  return parsedValue >= 0 ? parsedValue : ''
}

export const lowerPriority = (src: string, offset: number, rules: RuleMap): boolean => {
...
export const getAttributes = (html: string): AttributeMap | null => {
  ...
  for (const attr of target.getAttributeNames()) {
    ...
    if (/width|height/.test(attr)) {
      ...
      attrs[attr] = target.getAttribute(attr) as string
    }
  }
...
export const parseSrcAndTitle = (text = ''): ParsedSrcAndTitle => {
...
const canOpenEmphasis = (src: string, marker: string, pending: string): boolean => {
...
const canCloseEmphasis = (src: string, offset: number, marker: string): boolean => {
