...
import type { BlockLike, RenderVNode, SnabbdomHelper } from './types'

type FunctionTypeIconKey =
  | 'mermaid'
  | 'flowchart'
  | 'sequence'
  | 'plantuml'
...
const FUNCTION_TYPE_HASH: Record<FunctionTypeIconKey, string> = {
  mermaid: mermaidIcon,
...
const resolveFunctionIcon = (functionType?: string): string | undefined => {
  if (!functionType) {
    return undefined
  }

  return FUNCTION_TYPE_HASH[functionType as FunctionTypeIconKey]
}

export default function renderIcon (block: BlockLike): RenderVNode {
  if (block.parent) {
  ...
  switch (type) {
    ...
    case 'figure':
    case 'pre': {
      icon = resolveFunctionIcon(functionType) ?? null
      if (!icon) {
