...
type PreBlockHash = Record<string, string>

const PRE_BLOCK_HASH: PreBlockHash = {
...
export default function renderContainerBlock (
  ...
  const attrs: Record<string, SnabbdomDataValue> = {}
  const dataset: Record<string, SnabbdomDataValue> = {}
  ...
  if (/^(?:th|td)$/.test(type)) {
    ...
    if (cells && cells.length) {
      ...
      if (findTable && renderingRowContainer) {
        const { row: tableRow = 0, column: tableColumn = 0 } = findTable
    ...
    if (functionType) {
      Object.assign(dataset, { role: functionType.toUpperCase() })
    ...
    if (type === 'ol') {
      Object.assign(attrs, { start: block.start })
    }
  } else if (type === 'li' && listItemType) {
    Object.assign(dataset, { marker: bulletMarkerOrDelimiter })
    ...
    if (typeof functionType === 'string') {
      Object.assign(dataset, { role: functionType })
    }
    selector += PRE_BLOCK_HASH[block.functionType as keyof PreBlockHash] || ''

    if (typeof functionType === 'string' && /html|multiplemath|mermaid|flowchart|vega-lite|sequence|plantuml/.test(functionType)) {
  ...
  return h(selector, data as unknown as Parameters<typeof h>[1], children)
}
