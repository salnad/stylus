...
type QuickInsertIcon = string

export interface QuickInsertMenuItem {
  title: string
  subTitle: string
  label: string
  shortCut?: string
  icon: QuickInsertIcon
}

type QuickInsertGroup =
  | 'basic block'
  | 'header'
  | 'advanced block'
  | 'list block'
...
export type QuickInsertConfig = Record<QuickInsertGroup, QuickInsertMenuItem[]>

const COMMAND_KEY = isOsx ? '⌘' : 'Ctrl'
const OPTION_KEY = isOsx ? '⌥' : 'Alt'
const SHIFT_KEY = isOsx ? '⇧' : 'Shift'

export const quickInsertObj: QuickInsertConfig = {
  'basic block': [{
    title: 'Paragraph',
