...
type PasteType = 'MERGE' | 'NEWLINE'
type CopyType = 'normal' | 'copyAsMarkdown' | 'copyAsHtml' | 'htmlToMd'
type PasteHandlerType = 'normal' | 'pasteAsPlainText'
...
interface ContentStateLike {
  ...
  selectedImage: Pick<ImageInfoLike, 'key' | 'token'> | null
  ...
  updateCodeLanguage(block: BlockLike, language: string): void
...
interface PasteCtrlMethods {
  checkPasteType(start: BlockLike, fragment: BlockLike): PasteType
  checkCopyType(html: string, rawText: string): CopyType
  standardizeHTML(rawHtml: string): Promise<string>
  pasteImage(event: ClipboardEvent): Promise<string | File | null>
  docPasteHandler(event: ClipboardEvent): Promise<void>
  pasteHandler(
    event: ClipboardEvent,
    type?: PasteHandlerType,
    rawText?: string,
    rawHtml?: string
  ): Promise<unknown>
}
...
const pasteCtrl = (ContentState: ContentStateConstructor): void => {
  const prototype = ContentState.prototype as ContentStateLike & PasteCtrlMethods

  // check paste type: `MERGE` or `NEWLINE`
  prototype.checkPasteType = function (start: BlockLike, fragment: BlockLike): PasteType {
  ...
  prototype.checkCopyType = function (html: string, rawText: string): CopyType {
    let type: CopyType = 'normal'
    if (!html && rawText) {
      type = 'copyAsMarkdown'
  ...
  prototype.standardizeHTML = async function (rawHtml: string): Promise<string> {
    ...
    const tables = Array.from(tempWrapper.querySelectorAll<HTMLTableElement>('table'))
    for (const table of tables) {
      const row = table.querySelector('tr') as HTMLTableRowElement
      if (row.firstElementChild?.tagName !== 'TH') {
        Array.from(row.children).forEach(cell => {
          const th = document.createElement('th')
          th.innerHTML = (cell as HTMLElement).innerHTML
          cell.replaceWith(th)
        })
      }
      const paragraphs = Array.from(table.querySelectorAll<HTMLParagraphElement>('p'))
    ...
    const links = Array.from(tempWrapper.querySelectorAll<HTMLAnchorElement>('a'))
    for (const link of links) {
      const href = link.getAttribute('href') as string
      const text = link.textContent as string
  ...
  prototype.pasteImage = async function (event: ClipboardEvent): Promise<string | File | null> {
    ...
    if (file) {
      ...
      reader.onload = (loadEvent: ProgressEvent<FileReader>) => {
        ...
        const imageWrapper = this.muya.container.querySelector<HTMLElement>(`span[data-id=${id}]`) as HTMLElement
        const imageContainer = this.muya.container.querySelector<HTMLElement>(`span[data-id=${id}] .ag-image-container`)
  ...
  prototype.docPasteHandler = async function (event: ClipboardEvent): Promise<void> {
  ...
  prototype.pasteHandler = async function (
    event: ClipboardEvent,
    type: PasteHandlerType = 'normal',
    rawText?: string,
    rawHtml?: string
  ): Promise<unknown> {
    ...
    const text = rawText || clipboardData.getData('text/plain')
    let html = rawHtml || clipboardData.getData('text/html')
    ...
    const appendHtml = (text: string): void => {
    ...
    if (copyType === 'copyAsHtml') {
      switch (type) {
        ...
        case 'pasteAsPlainText': {
          ...
          let htmlBlock: BlockLike | null = null
    ...
    const getLastBlock = (blocks: BlockLike[]): BlockLike => {
      const len = blocks.length
      const lastBlock = blocks[len - 1] as BlockLike

      if (lastBlock.children.length === 0 && HAS_TEXT_BLOCK_REG.test(lastBlock.type)) {
        return lastBlock
      } else {
        if (lastBlock.editable === false) {
          return getLastBlock((blocks[len - 2] as BlockLike).children)
        } else {
    ...
    switch (pasteType) {
      case 'MERGE': {
        if (LIST_REG.test(firstFragment.type)) {
          ...
          const originListItem = this.getParent(parent) as BlockLike
          const originList = this.getParent(originListItem) as BlockLike
