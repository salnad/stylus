...
type ListIndentationMode = 'number' | 'dfm'
type ListContainerType = 'ul' | 'ol'
type TableAlignment = 'left' | 'center' | 'right' | string
...
interface TextBlock extends MarkdownBlock {
  text: string
}

interface HeadingBlock extends MarkdownBlock {
  children: [TextBlock, ...MarkdownBlock[]]
  headingStyle?: string
  marker?: string
}

interface FigureBlock extends MarkdownBlock {
  children: [MarkdownBlock, ...MarkdownBlock[]]
}

interface FrontMatterBlock extends MarkdownBlock {
  lang?: string
  style?: string
  children: [MarkdownBlock]
}

interface MultipleMathBlock extends MarkdownBlock {
  mathStyle?: string
  children: [MarkdownBlock]
}

interface ContainerBlock extends MarkdownBlock {
  children: [MarkdownBlock]
}

interface CodeBlock extends MarkdownBlock {
  lang?: string
  children: [MarkdownBlock, MarkdownBlock]
}

interface ListItemBlock extends MarkdownBlock {
  children: MarkdownBlock[]
  isLooseListItem?: boolean
  bulletMarkerOrDelimiter?: string
  listItemType?: string
...
interface ListBlock extends MarkdownBlock {
  children: ListItemBlock[]
  start?: number
}

interface FootnoteBlock extends MarkdownBlock {
  children: [TextBlock, MarkdownBlock, ...MarkdownBlock[]]
}

interface TableCellBlock extends MarkdownBlock {
  align?: TableAlignment
  children: [TextBlock, ...MarkdownBlock[]]
}

interface TableRowBlock extends MarkdownBlock {
  children: TableCellBlock[]
}

interface TableSectionBlock extends MarkdownBlock {
  children: TableRowBlock[]
}

interface TableBlock extends MarkdownBlock {
  row: number
  column: number
  children: TableSectionBlock[]
}
...
class ExportMarkdown {
  ...
  listType: ListMetadata[]
  isLooseParentList: boolean
  isGitlabCompatibilityEnabled: boolean
  listIndentation: ListIndentationMode
  listIndentationCount: number

  constructor (blocks: MarkdownBlock[], listIndentation: ListIndentationInput = 1, isGitlabCompatibilityEnabled = false) {
    this.blocks = blocks
  ...
  translateBlocks2Markdown (blocks: MarkdownBlock[], indent = '', listIndent = ''): string {
    const result: string[] = []
    ...
    for (const block of blocks) {
      ...
      switch (block.type) {
        ...
        case 'span': {
          result.push(this.normalizeParagraphText(block as TextBlock, indent))
          break
        }
        case 'h1':
        case 'h2':
        ...
        case 'h6': {
          this.insertLineBreak(result, indent)
          result.push(this.normalizeHeaderText(block as HeadingBlock, indent))
          break
        }
        case 'figure': {
          this.insertLineBreak(result, indent)
          switch (block.functionType) {
            case 'table': {
              const table = (block as FigureBlock).children[0] as TableBlock
              result.push(this.normalizeTable(table, indent))
              break
            }
            case 'html': {
              result.push(this.normalizeHTML(block as FigureBlock, indent))
              break
            }
            case 'footnote': {
              result.push(this.normalizeFootnote(block as FootnoteBlock, indent))
              break
            }
            case 'multiplemath': {
              result.push(this.normalizeMultipleMath(block as MultipleMathBlock, indent))
              break
            }
            case 'mermaid':
            case 'flowchart':
            ...
            case 'plantuml':
            case 'vega-lite': {
              result.push(this.normalizeContainer(block as ContainerBlock, indent))
              break
            }
          }
          break
        ...
        case 'li': {
          ...
          result.push(this.normalizeListItem(block as ListItemBlock, indent + listIndent))
          this.isLooseParentList = true
          break
        }
        case 'ul': {
          ...
          const firstChild = (block as ListBlock).children[0]
          ...
          result.push(this.normalizeList(block as ListBlock, indent, listIndent))
          this.listType.pop()
          break
        }
        case 'ol': {
        ...
        case 'pre': {
          ...
          if (block.functionType === 'frontmatter') {
            result.push(this.normalizeFrontMatter(block as FrontMatterBlock, indent))
          } else {
            result.push(this.normalizeCodeBlock(block as CodeBlock, indent))
          }
          break
        }
        case 'blockquote': {
  ...
  insertLineBreak (result: string[], indent: string): void {
    if (!result.length) return
    result.push(`${indent}\n`)
  }

  normalizeParagraphText (block: TextBlock, indent: string): string {
    const { text } = block
    const lines = text.split('\n')
    return lines.map(line => `${indent}${line}`).join('\n') + '\n'
  }

  normalizeHeaderText (block: HeadingBlock, indent: string): string {
    const { headingStyle, marker } = block
    const { text } = block.children[0]
    if (headingStyle === 'atx') {
      const match = text.match(/(#{1,6})(.*)/) as RegExpMatchArray
  ...
  normalizeBlockquote (block: MarkdownBlock, indent: string): string {
    const { children } = block
    const newIndent = `${indent}> `
    return this.translateBlocks2Markdown(children, newIndent)
  }

  normalizeFrontMatter (block: FrontMatterBlock, _indent: string): string { // preBlock
    let startToken = ''
    let endToken = ''
    switch (block.lang) {
      case 'yaml':
    ...
    for (const line of codeBlock.children[0].children) {
      const textLine = line as TextBlock
      result.push(`${textLine.text}\n`)
    }
    result.push(endToken)
    return result.join('')
  }

  normalizeMultipleMath (block: MultipleMathBlock, /* figure */ indent: string): string {
    ...
    for (const line of block.children[0].children[0].children) {
      const mathLine = line as TextBlock
      result.push(`${indent}${mathLine.text}\n`)
    }
  ...
  normalizeContainer (block: ContainerBlock, _indent: string): string {
    const result: string[] = []
    const diagramType = block.children[0].functionType || ''
    result.push('```' + diagramType + '\n')
    for (const line of block.children[0].children[0].children) {
      const containerLine = line as TextBlock
      result.push(`${containerLine.text}\n`)
    }
    result.push('```\n')
    return result.join('')
  }

  normalizeCodeBlock (block: CodeBlock, indent: string): string {
    const result: string[] = []
    const codeContent = block.children[1].children[0] as TextBlock
  ...
  normalizeHTML (block: FigureBlock, indent: string): string { // figure
    const result: string[] = []
    const codeContentText = ((block.children[0].children[0].children[0]) as TextBlock).text
  ...
  normalizeTable (table: TableBlock, indent: string): string {
    const result: string[] = []
    ...
    tableData.push(tHeader.children[0].children.map(th => escapeText((th.children[0] as TextBlock).text).trim()))
    if (tBody) {
      tBody.children.forEach(bodyRow => {
        tableData.push(bodyRow.children.map(td => escapeText((td.children[0] as TextBlock).text).trim()))
      })
    }

    const columnWidth: TableColumnMetadata[] = tHeader.children[0].children.map(th => ({ width: 5, align: th.align }))
    ...
    for (i = 0; i <= row; i++) {
      for (j = 0; j <= column; j++) {
        const currentColumn = columnWidth[j] as TableColumnMetadata
        currentColumn.width = Math.max(currentColumn.width, (tableData[i] as string[])[j].length + 2) // add 2, because have two space around text
      }
    }
    tableData.forEach((r, i) => {
      const rs = indent + '|' + r.map((cell, j) => {
        const raw = ` ${cell + ' '.repeat((columnWidth[j] as TableColumnMetadata).width)}`
        return raw.substring(0, (columnWidth[j] as TableColumnMetadata).width)
      }).join('|') + '|'
      result.push(rs)
      if (i === 0) {
  ...
  normalizeList (block: ListBlock, indent: string, listIndent: string): string {
    const { children } = block
    return this.translateBlocks2Markdown(children, indent, listIndent)
  }

  normalizeListItem (block: ListItemBlock, indent: string): string {
    const result: string[] = []
    const listInfo = this.listType[this.listType.length - 1] as ListMetadata
    ...
    if (isUnorderedList) {
      ...
      let n = listInfo.listCount as number
      if ((this.listIndentation === 'dfm' && n > 99) || n > 999999999) {
        n = 1
      }
      listInfo.listCount = (listInfo.listCount as number) + 1
    ...
    if (isUnorderedList && block.listItemType === 'task') {
      const firstChild = children[0] as MarkdownBlock
  ...
  normalizeFootnote (block: FootnoteBlock, indent: string): string {
    const result: string[] = []
    ...
    if (hasMultipleBlocks) {
      ...
      result.push(' ')
      const paragraphContent = block.children[1].children[0] as TextBlock
