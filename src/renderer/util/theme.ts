...
type ThemeName = 'light' | 'dark' | 'graphite' | 'material-dark' | 'one-dark' | 'ulysses'
...
export const addThemeStyle = (theme: ThemeName | string): void => {
  const isCmRailscasts = (railscastsThemes as readonly string[]).includes(theme)
  const isCmOneDark = (oneDarkThemes as readonly string[]).includes(theme)
  const isDarkTheme = isCmOneDark || isCmRailscasts
  let themeStyleEle = document.querySelector<HTMLStyleElement>(`#${THEME_STYLE_ID}`)
  if (!themeStyleEle) {
  ...
  const cm = document.querySelector<HTMLElement>('.CodeMirror')
  if (cm) {
...
export const setEditorWidth = (value: string): void => {
  ...
  let styleEle = document.querySelector<HTMLStyleElement>(`#${EDITOR_WIDTH_STYLE_ID}`)
  if (!styleEle) {
...
export const addCommonStyle = (options: CommonStyleOptions): void => {
  ...
  let sheet = document.querySelector<HTMLStyleElement>(`#${COMMON_STYLE_ID}`)
  if (!sheet) {
...
export const addElementStyle = (): void => {
  ...
  let sheet = document.querySelector<HTMLStyleElement>(`#${ID}`)
  if (sheet) {
    return
  }
...
export const addStyles = (options: CommonStyleOptions): void => {
  const { theme } = options
  if (theme) {
    addThemeStyle(theme)
  }
