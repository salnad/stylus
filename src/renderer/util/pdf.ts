...
export const getCssForOptions = (options: PdfOptions): string => {
  const {
    type,
    pageMarginTop,
    pageMarginRight,
...
const generateHtmlToc = (
  ...
  const { content, lvl } = tocList.shift() as TocItem
...
export const getHtmlToc = (toc: TocItem[], options: PdfOptions = {}): string => {
  const list = cloneObj(toc) as TocItem[]
