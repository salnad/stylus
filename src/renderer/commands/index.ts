...
export interface CommandLike {
  ...
  execute(): Promise<void>
  executeSubcommand?(id?: string, value?: unknown): Promise<void>
}

export interface StaticCommandEntry {
  id: string
  description?: string
  shortcut?: string[] | null
  value?: unknown
...
export class RootCommand implements CommandLike {
  id: string
  description: string
  placeholder?: string
  shortcut?: string[] | null
...
const focusEditorAndExecute = (handler: () => void): void => {
  setTimeout(() => bus.$emit('editor-focus'), 10)
  setTimeout(() => handler(), 150)
}

const commands: StaticCommandEntry[] = [
  {
    id: 'file.new-tab',
    execute: async () => {
    ...
    executeSubcommand: async (_id?: string, value?: unknown) => {
    ...
    executeSubcommand: async (_id?: string, theme?: unknown) => {
